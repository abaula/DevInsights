...
```
🔒 Запросил объект → получил «ключ» → поработал → отпустил ключ → следующий может зайти
```

Всё построено на двух простых классах из `Shared.Infrastructure`:

### 1️⃣ LockedValueFactory<T> — создаёт «блокируемые» объекты
...
### 2️⃣ LockedValue<T> — обёртка, которая «отпускает ключ» автоматически
...
✨ **Магия в том**, что `LockedValue<T>` реализует `IDisposable`. Это значит, что его можно использовать с конструкцией `using`, и ключ освободится **автоматически**, даже если в коде возникнет ошибка.
...
class FetchJobsToMemory : IFetchJobsToMemory
{
    ...
    {
        // 🔑 Получаем эксклюзивный доступ к коллекции
        ...
        var activeJobItems = await _selectJobsItems.Value.Execute(...);

        jobCollectionValue.Value.Update(jobCollectionData.ToArray());

        // 🎉 Выход из using → автоматический Dispose() → ключ свободен
    }
}
```
...
1️⃣ Вызывается FetchJobsToMemory.Execute()
        ↓
2️⃣ Create() ждёт, пока семафор станет свободным
        ↓
3️⃣ Семафор захвачен → создаётся LockedValue<IJobCollection>
        ↓
4️⃣ Выполняется работа с объектом (чтение/запись)
        ↓
5️⃣ Завершается блок using → вызывается Dispose()
        ↓
...
Попробуй применить этот паттерн в своём проекте — и спи спокойно, зная, что твои данные под защитой! 🛡️
